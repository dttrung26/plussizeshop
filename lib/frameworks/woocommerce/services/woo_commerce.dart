...
class WooCommerce implements BaseServices {
  ...
  void appConfig(appConfig) {
    ...
    categoryCache = Map<String, List<Product>>();
  }
  ...
  Future<List<Category>> getCategories({lang}) async {
    try {
      ...
      while (!isEnd) {
        List<Category> categories =
            await getCategoriesByPage(lang: lang, page: page);
    ...
    try {
      ...
      for (var i = 1; i < 100; i++) {
        List<SubAttribute> subAttributes =
            await getSubAttributesByPage(id: id, page: i);
  ...
  Future<List<SubAttribute>> getSubAttributesByPage({id, page}) async {
    try {
      List<SubAttribute> listAttributes = [];

      String url = 'products/attributes/$id/terms?per_page=100&page=$page';
      var response = await wcApi.getAsync(url);
...
//          }
  ...
  Future<List<Product>> searchProducts({
    ...
    try {
      String endPoint =
          "products?status=publish&page=$page&per_page=$ApiPageSize";
  ...
  Future<Product> getProduct(id, {lang}) async {
    try {
      String endpoint = (lang == null || !kAdvanceConfig["isMultiLanguages"])
          ? "products/$id"
            ...
            for (var j = 0; j < items.length; j++) {
              ...
              if (prods != null && prods.isNotEmpty) {
                for (var prod in prods) {
                  listProduct.add(jsonParser(prod));
                }
              }
  ...
  Future<Map<String, dynamic>> getCurrencyRate() async {
    try {
      ...
      if (response.statusCode == 200 && body != null && body is Map) {
        Map<String, dynamic> data = Map.from(body);
        Map<String, dynamic> currency = Map<String, dynamic>();
  ...
  Future<Map<String, dynamic>> getTaxes(CartModel cartModel) async {
    try {
      ...
      final http.Response response = await http.post(
          "$url/wp-json/api/flutter_woo/taxes",
          body: convert.jsonEncode(params));
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 200) {
        List<Tax> taxes = [];
  ...
  Future<Map<String, Tag>> getTagsByPage({String lang, int page}) async {
    try {
      String url = "products/tags?per_page=100&page=$page&hide_empty=true";
      if (lang != null && kAdvanceConfig["isMultiLanguages"]) {
        url += "&lang=$lang";
      }
      var response = await wcApi.getAsync(url);
  ...
  Future<Map<String, Tag>> getTags({String lang}) async {
    try {
      ...
      while (!isEnd) {
        Map<String, Tag> _tags = await getTagsByPage(lang: lang, page: page);
        if (_tags.isEmpty) {
          isEnd = true;
        }
        page = page + 1;
  ...
  Future<Product> createProduct(
    ...
    try {
      final response =
          await wcApi.postAsync('flutter/product', {...data, "cookie": cookie});
      if (response["message"] == null) {
        return jsonParser(response);
      } else {
        throw Exception(response["message"]);
  ...
  Future<List<Product>> getOwnProducts(String cookie,
    ...
    try {
      final response = await wcApi.postAsync(
          'flutter/products/owner', {"cookie": cookie, "page": page});
      if (response is Map && isNotBlank(response["message"])) {
        throw Exception(response["message"]);
      } else {
        List<Product> list = [];
  ...
  Future<dynamic> uploadImage(dynamic data) async {
    try {
      final response = await wcApi.postAsync("flutter/media", data);
  ...
  Future updatePoints(String token, Order order) async {
    try {
      final http.Response response = await http.patch(
          "$url/wp-json/api/flutter_woo/points",
  ...
  Future<List<StaffBooking>> getListStaff(String idProduct) async {
    ...
    final http.Response response = await http.get(urlAPI);
